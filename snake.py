"""
This file is based on the following open-source project:
Project name: snake-ai
Original Project URL: https://github.com/linyiLYi/snake-ai
...
class SnakeGame:
    ...
    def draw_snake(self):
        ...
        #     body_radius = 5
        #     # Yellow to light red gradient
        #     pygame.draw.rect(self.screen, (255, color_list[i], 0),
        #                     (body_x, body_y, body_width, body_height))
        #     i += 1
        color_list = np.linspace(255, 100, len(self.snake) - 1, dtype=np.uint8)
        i = 1
        for index, (r, c) in enumerate(self.snake[1:]):
            body_x = c * self.cell_size + self.border_size
            body_y = r * self.cell_size + self.border_size
            body_width = self.cell_size
            body_height = self.cell_size
            body_radius = 5
            if index == 0:
                # First body segment (second segment of the snake)
                color = (255, 255, 0)
            elif index == len(self.snake) - 2:
                # Last body segment
                color = (255, 100, 0)
            else:
                # Gradient color
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if game_state == "running":
            # Update game state when new input is received
            if has_new_input:
                done, _ = game.step(action)
                game.render()
